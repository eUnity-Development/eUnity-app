...
client = openai.Client(api_key=os.getenv("GROQ_API_KEY"),
...
def save_python(path: str, code: str) -> dict:
    ...
    ## save the file
    with open(f"{sub_dir}/{path}", "w") as f:
        f.write(code)
    return {"message": "File saved."}


@ell.complex(model="llama3-groq-70b-8192-tool-use-preview", tools=[save_python], client=client)
def to_file(message: str, save_name: str) -> str:
    return [
        ell.user(f"Save the following code to a file using the save_python tool, and path {save_name}" + message)
    ]
    ...
    for file in files:
        with open(f"{file}.go", "r") as f:
            ...
            if response.content[0].tool_call is not None:
                ...
                save_python(response.content[0].tool_call.params.path, response.content[0].tool_call.params.code)

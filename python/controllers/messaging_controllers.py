...
async def send_message(message: str = Query(...), user_id: str = Query(...), to_user_id: str = Query(...)):
    from_user_id = user_id
    conversation_id = "placeholder"  # Replace with actual conversation ID
    message_obj = Message(conversation_id=conversation_id, from_user=from_user_id, to_user=to_user_id, message=message)
    try:

...
async def generate_mock_users(amount: int):
    ...
    if err !=  None:
        return JSONResponse(status_code=400, content={"response": "Error creating mock users"})

    # always return a random user email that I am able to login with
    return JSONResponse(status_code=200, content={"response": f"{amount} mock users created", "random_user_email": user.__dict__["email"]})
